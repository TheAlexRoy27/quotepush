...
const botRouter = router({
  ...
  saveConfig: protectedProcedure
    .input(z.object({
      ...
      tone: z.enum(["friendly", "professional", "casual", "empathetic", "direct", "karen", "kevin", "brianna", "kayla", "marcus"]).optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      const systemPrompt = [
        input.identity?.replace(/\{botName\}/g, botName) ??
          `You are ${botName}, an insurance advisor helping leads get a quote.`,
        `PERSONA AND VOICE - this overrides everything else about how you write: ${toneGuide[input.tone ?? "friendly"]}`,
